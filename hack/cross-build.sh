...
for i in "${build_matrix[@]}"; do
  ...
  -ldflags "-s -w \
            ...
            -X ocm.software/ocm-core/api/version.gitTreeState=$([ -z "$(git status --porcelain 2>/dev/null)" ] && echo clean || echo dirty) \
            -X ocm.software/ocm-core/api/version.gitCommit=$(git rev-parse --verify HEAD) \
